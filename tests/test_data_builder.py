...
class DataBuilderTestCase(unittest.TestCase):
    ...
    def test_training_df_basic(self):
        df_builder = data_builder.DataFrameBuilder({})

        y_df = df_builder.training_df(self.df, "y")
        np.testing.assert_array_equal(["ds", "y"], y_df.columns)

        y1_df = df_builder.training_df(self.df, "y1")
        np.testing.assert_array_equal(["ds", "y"], y1_df.columns)

    def test_capacity_floor_df(self):
        self.df["floor_y"] = 50
        self.df["cap_y"] = 500
        self.df["cap_y1"] = 500
